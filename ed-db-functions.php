...
function lti_maybe_create_db() {
	...
	if ( is_user_logged_in() && is_super_admin() ) {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->ltitable}'" ) != $wpdb->ltitable ) {
			...
			$wpdb->query(
				"CREATE TABLE IF NOT EXISTS `{$wpdb->ltitable}` (
			...
			$wpdb->query(
				"ALTER TABLE `{$wpdb->ltitable}`
                ADD UNIQUE INDEX {$wpdb->ltitable}_consumer_key_UNIQUE (consumer_key256 ASC);"
			);

			$wpdb->query(
				'CREATE TABLE IF NOT EXISTS `' . $wpdb->base_prefix . 'lti2_tool_proxy` (
			...
			$wpdb->query(
				'ALTER TABLE `' . $wpdb->base_prefix . "lti2_tool_proxy`
			...
			$wpdb->query(
				'CREATE TABLE IF NOT EXISTS `' . $wpdb->base_prefix . 'lti2_nonce` (
                  consumer_pk int(11) NOT NULL,
			...
			$wpdb->query(
				'ALTER TABLE `' . $wpdb->base_prefix . "lti2_nonce`
			...
			$wpdb->query(
				'CREATE TABLE IF NOT EXISTS `' . $wpdb->base_prefix . 'lti2_context` (
			...
			$wpdb->query(
				'ALTER TABLE `' . $wpdb->base_prefix . "lti2_context`
			...
			$wpdb->query(
				'CREATE TABLE IF NOT EXISTS `' . $wpdb->base_prefix . 'lti2_resource_link` (
			...
			$wpdb->query(
				'ALTER TABLE `' . $wpdb->base_prefix . "lti2_resource_link`
			...
			$wpdb->query(
				'CREATE TABLE IF NOT EXISTS `' . $wpdb->base_prefix . 'lti2_user_result` (
			...
			$wpdb->query(
				'ALTER TABLE `' . $wpdb->base_prefix . "lti2_user_result`
			...
			$wpdb->query(
				'CREATE TABLE IF NOT EXISTS `' . $wpdb->base_prefix . 'lti2_share_key` (
			...
			$wpdb->query(
				'ALTER TABLE `' . $wpdb->base_prefix . "lti2_share_key`
...
function lti_maybe_create_site_blogs_meta_table() {
	...
	if ( is_user_logged_in() && is_super_admin() ) {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->blogsmetatable}'" ) != $wpdb->blogsmetatable ) {
			...
			$wpdb->query(
				"CREATE TABLE IF NOT EXISTS `{$wpdb->blogsmetatable}` (
			...
			$wpdb->query(
				"ALTER TABLE `{$wpdb->blogsmetatable}`
